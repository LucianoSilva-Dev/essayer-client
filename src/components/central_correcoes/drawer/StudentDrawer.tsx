...
import { AlunoSubmissaoMock } from "../types";

const MOCK_ALUNOS: AlunoSubmissaoMock[] = Array.from({ length: 15 }).map((_, i) => ({
  id: `aluno-${i}`,
  nome: `Estudante Mock ${i + 1}`,
  status: i < 5 ? 'corrigido' : 'pendente',
  dataEnvio: "11/04/2025",
  tempoGasto: "54 min",
  nota: i < 5 ? 800 + (i * 20) : undefined
}));
...
export function StudentDrawer({ isOpen, onClose, tarefaId }: Props) {
  ...
  const filteredAlunos = MOCK_ALUNOS.filter(aluno => {
    const matchesSearch = aluno.nome.toLowerCase().includes(searchTerm.toLowerCase());
    if (!matchesSearch) return false;
    if (filter === 'todos') return true;
    return aluno.status === filter;
  });
  ...
  return (
    <>
      <div 
        className={`fixed inset-0 bg-[#075F70]/20 backdrop-blur-[2px] z-40 transition-opacity duration-300 ${
            isOpen ? "opacity-100" : "opacity-0 pointer-events-none"
        }`} 
        onClick={handleClose}
      />

      <div className={`fixed inset-y-0 right-0 w-full md:w-[550px] bg-white shadow-2xl z-50 transform transition-transform duration-300 ease-out flex flex-col ${
                ...
                filteredAlunos.map((aluno) => (
                    ...
                    >
                        ...
                        <div className="flex items-center gap-4 w-full sm:w-auto">
                            ...
                            <div>
                                <p className="font-bold text-gray-800 text-base">{aluno.nome}</p>
                        ...
                        <div className="flex items-center justify-between sm:justify-end gap-6 w-full sm:w-auto mt-2 sm:mt-0 pl-16 sm:pl-0">
                            ...
                            <div className="text-right hidden sm:block">
                                ...
                                <p className="text-xs font-medium text-gray-600">{aluno.dataEnvio}</p>
                            </div>
        ...
        <div className="px-6 py-4 border-t border-gray-100 bg-white flex justify-between items-center text-xs font-medium text-gray-500 shrink-0">
            <span>Total: {MOCK_ALUNOS.length}</span>
            <div className="flex gap-4">
                <span>Corrigidos: {MOCK_ALUNOS.filter(a => a.status === 'corrigido').length}</span>
                <span>Pendentes: {MOCK_ALUNOS.filter(a => a.status === 'pendente').length}</span>
            </div>
        </div>
