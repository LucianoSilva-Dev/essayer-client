...
export function TarefaGrid({ tarefas, onSelectTarefa }: Props) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 pb-20">
      {tarefas.map((tarefa) => {
        const extraAlunos = tarefa.entregues > 3 ? tarefa.entregues - 3 : 0;
        const totalAvataresParaMostrar = Math.min(tarefa.entregues, 3);
        
        // Verificações de Status
        const isEncerrada = tarefa.status === 'encerrada';
        const textoData = isEncerrada ? 'Fechou em' : 'Fecha em';
        ...
        >
          ...
          <div className="relative">
            ...
            <div className="flex justify-between items-start mb-2 h-10">
                <span className={`text-sm font-medium block mt-1 transition-colors ${isEncerrada ? 'text-gray-500' : 'text-gray-500 group-hover:text-[#075F70]'}`}>
                    {tarefa.tipo}
                </span>
          ...
          <div className="pt-4 border-t border-gray-200 flex justify-between h-25">
            ...
            <div className="flex flex-col justify-between">
                ...
                <div className="flex items-center relative pb-1">
                    <div className="flex -space-x-5">
                        {[...Array(totalAvataresParaMostrar)].map((_, i) => (
                             <div 
                                key={i} 
                                style={{ zIndex: i * 10 }}
                    ...
                    {extraAlunos > 0 && (
                        <div 
                            ...
                            className={`relative -ml-4 h-9 min-w-[3.5rem] px-1.5 bg-white rounded-full text-[10px] font-bold shadow-sm border-2 border-white flex flex-col items-center justify-center leading-none text-center
                                ${isEncerrada ? "text-gray-500" : "text-[#075F70]"}
                            `}
                        >
            ...
            <div className="text-right flex flex-col items-end justify-between h-full">
                <span className={`text-xl font-bold leading-none mt-1 transition-transform origin-right group-hover:scale-105
                    ${isEncerrada ? "text-gray-600" : "text-[#075F70]"}
                `}>
                    {tarefa.entregues}<span className="text-gray-400 text-lg">/{tarefa.total}</span>
                </span>

                <span className="text-xs text-gray-500 font-medium leading-tight pb-1">
                    {textoData} <br/>{tarefa.prazo}
                </span>
            </div>
