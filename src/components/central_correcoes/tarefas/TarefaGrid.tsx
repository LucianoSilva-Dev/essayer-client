...
export function TarefaGrid({ tarefas, onSelectTarefa }: Props) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 pb-20">
      {tarefas.map((tarefa) => {
        const extraAlunos = tarefa.usuariosResponderam.length > 3 ? tarefa.usuariosResponderam.length - 3 : 0;
        const avataresParaMostrar = tarefa.usuariosResponderam.slice(0, 3);

        const isEncerrada = new Date(tarefa.dataLimite || '') < new Date()
        const textoData = tarefa.dataLimite ? isEncerrada ? 'Fechou em' : 'Fecha em' : ''
          ...
          >
            ...
            <div className="pt-4 border-t border-gray-200 flex justify-between h-25">
              ...
              <div className="text-right flex flex-col items-end justify-between h-full">
                ...
                `}>
                  {tarefa.usuariosResponderam.length}<span className="text-gray-400 text-lg">/{tarefa.totalMembros}</span>
                </span>

                <span className="text-xs text-gray-500 font-medium leading-tight pb-1">
                  {textoData} <br />{formatDate(tarefa.dataLimite)}
                </span>
              </div>
