...
const ChevronLeft = () => (
  <svg
    ...
    strokeWidth={3}
    stroke="currentColor"
    className="w-8 h-8 text-brand-teal-dark"
  >
    <path
      strokeLinecap="round"
...
export function TarefasAtivasList({
  ...
  if (loading) {
    return (
      <div className="h-[280px] w-full bg-gray-200 rounded-[40px] animate-pulse" />
    );
  }
  ...
  const proximaTarefa =
    tarefas.length > 1 ? tarefas[(currentIndex + 1) % tarefas.length] : null;
        ...
        <div className="relative w-full h-full md:max-w-[95%] lg:max-w-[650px] mx-auto perspective-1000">
          ...
          {proximaTarefa && (
            <div
              ...
              aria-hidden="true"
            >
              <div className="absolute bottom-0 right-0 h-[150px] w-[60%] bg-gradient-to-tl from-brand-teal-dark/20 to-transparent rounded-br-[40px] rounded-tl-[100px]" />
            </div>
          )}
          ...
          <div className="relative w-full min-h-[300px] h-full bg-white rounded-[40px] shadow-[0_8px_30px_rgba(0,0,0,0.08)] z-10 overflow-hidden border border-gray-50 flex flex-col justify-between">
            ...
            <div className="p-6 md:p-8 flex flex-col gap-2 relative z-20 w-full md:max-w-[55%] h-full justify-between">
              <div>
                ...
                <div className="mt-2 md:mt-4 flex flex-col gap-1">
                  <span className="font-montserrat font-bold text-neutral-dark text-xs md:text-sm uppercase tracking-wide">
                    {tarefaAtual.tipoAtividade}
                  </span>
                  <h1 className="font-montserrat font-medium text-neutral-dark text-xl md:text-[22px] leading-tight line-clamp-3">
                    {tarefaAtual.titulo}
                  </h1>
                </div>
              </div>
              ...
              <div className="pt-4 md:pb-8">
                <p className="font-montserrat font-semibold text-neutral-dark text-xs md:text-sm">
                  Fecha em{" "}
                  <span className="text-gray-600">
                    {formatDate(tarefaAtual.dataLimite)}
                  </span>
                </p>
              </div>
              ...
              <div className="md:hidden mt-4">
                <Link
                  ...
                  className="bg-[#E5EFF0] hover:bg-gray-100 text-brand-teal-dark font-bold py-3 px-6 rounded-full transition-all duration-300 text-sm flex items-center justify-center gap-2 shadow-sm w-full"
                >
                  Iniciar tarefa
                </Link>
              </div>
            ...
            <div className="hidden md:block absolute bottom-[35px] right-[40%] lg:right-[200px] z-30 pointer-events-auto font-montserrat">
              <Link
                href={`/fazer-tarefa/${tarefaAtual.id}`}
                className="bg-[#E5EFF0] hover:bg-white text-brand-teal-dark font-bold py-2.5 px-6 rounded-full transition-all duration-300 text-sm flex items-center gap-2 transform hover:scale-105 shadow-sm whitespace-nowrap"
              >
                Iniciar tarefa
              </Link>
            </div>
