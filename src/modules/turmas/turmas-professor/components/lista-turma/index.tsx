...
export default function ListaTurmas() {
  ...
  const mudarPagina = async (nova: number) => {
    ...
    setIsLoading(true);
    const novaQuery =
      turmas?.paginacao.pagesUrl[nova - 1] ?? `limit=${limit}&page=${nova}`;
  ...
  const renderTurmas = (turmasData?: GetTurmasCriadasResponse) => {
    ...
    return turmasData.documentos.map((turma) => (
      ...
      >
        <div className="p-3 bg-white border border-gray-100 rounded-2xl shadow-sm transition-all duration-300 group-hover:shadow-md group-hover:border-brand-teal-dark/30 flex items-center gap-3 relative overflow-hidden">
        ...
        {isLoading ? (
          <div className="space-y-3 animate-pulse">
            {[1, 2, 3, 4, 5].map((i) => (
              <div
                key={i}
                className="p-3 bg-white border border-gray-100 rounded-2xl flex items-center gap-3"
              >
